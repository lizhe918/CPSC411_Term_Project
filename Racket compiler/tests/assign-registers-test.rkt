...
(let ([x   `(module
                ...
                (define L.addup.2
                    ...
                    (jump L.addup.2 x.1)))
                (jump L.addup.2))])
     (test-case "Preserve assigned call-undead variable -- bitwise-and"
     (check-match (assign-registers x)
                `(module
                  (,_ ... (assignment ()) ,_ ...)
                  (define L.addup.2
...
(let ([x   `(module
                ...
                (define L.addup.2
                    ...
                    (jump L.addup.2 x.1)))
                (jump L.addup.2))])
     (test-case "Preserve assigned call-undead variable -- bitwise-ior"
     (check-match (assign-registers x)
                `(module
                  (,_ ... (assignment ()) ,_ ...)
                  (define L.addup.2
...
(let ([x   `(module
                ...
                (define L.addup.2
                    ...
                    (jump L.addup.2 x.1)))
                (jump L.addup.2))])
     (test-case "Preserve assigned call-undead variable -- bitwise-xor"
     (check-match (assign-registers x)
                `(module
                  (,_ ... (assignment ()) ,_ ...)
                  (define L.addup.2
...
(let ([x   `(module
                ...
                (define L.addup.2
                    ...
                    (jump L.addup.2 x.1)))
                (jump L.addup.2))])
     (test-case "Preserve assigned call-undead variable -- arithmetic-shift-right"
     (check-match (assign-registers x)
                `(module
                  (,_ ... (assignment ()) ,_ ...)
                  (define L.addup.2
...
(let ([x   `(module
     ...
     (test-case "Assign all alocs when we have plenty assignable registers"
     (check-match (assign-registers x)
                `(module
...
(let ([x   `(module
                ...
                (define L.addup.2
                    ...
                    (begin (set! x.1 1) (set! x.2 L.addup.1) (jump x.2 x.1)))
                (begin (set! q.1 1) (set! m.1 2) (jump L.addup.2 q.1 m.1)))])
     (test-case "Assign no aloc when we have no assignable registers"
     (check-match (parameterize ([current-assignable-registers '()]) (assign-registers x))
                `(module
                (,_ ... (assignment ()) ,_ ...)
                (define L.addup.2
                  (,_ ... (assignment ()) ,_ ...)
...
(let ([x   `(module
                ...
                (define L.addup.2
                    ...
                    (begin (set! x.1 1) (set! x.2 L.addup.1) (jump x.2 x.1)))
                (begin (set! q.1 1) (set! m.1 2) (jump L.addup.2 q.1 m.1)))])
     (test-case "Assign partial alocs when we have limited assignable registers"
     (check-match (parameterize ([current-assignable-registers '(r15)]) (assign-registers x))
                `(module
                  (,_ ... (assignment ((m.1 r15))) ,_ ...)
                  (define L.addup.2
                    (,_ ... (assignment ((x.2 r15))) ,_ ...)
                    (begin (set! x.1 1) (set! x.2 L.addup.1) (jump x.2 x.1)))
                  (begin (set! q.1 1) (set! m.1 2) (jump L.addup.2 q.1 m.1))))))
...
(let ([x   `(module
                ...
                (define L.addup.2
                    ...
                    (jump L.addup.2 x.1)))
                (jump L.addup.2))])
     (test-case "Preserve assigned call-undead variable"
     (check-match (assign-registers x)
                `(module
                  (,_ ... (assignment ()) ,_ ...)
                  (define L.addup.2
...
(let ([x   `(module
                ...
                (define L.addup.2
                    ...
                    (jump L.addup.2 x.1)))
                (jump L.addup.2))])
     (test-case "Preserve allocated new frame variable"
     (check-match (parameterize ([current-assignable-registers '(r15 r14)]) (assign-registers x))
                `(module
                (,_ ... (assignment ()) ,_ ...)
                (define L.addup.2
                  (,_ ... (assignment
...
(let ([x   `(module
     ...
     (check-match (parameterize ([current-assignable-registers '(r15)]) (assign-registers x))
                `(module
                (,_ ... (assignment ((tmp-ra.4 r15))) ,_ ...)
                (define L.swap.1
                  (,_ ... (assignment ,(list-no-order '(tmp-ra.1 fv2) '(nfv.2 fv3) '(nfv.3 fv4) '(y.2 r15) '(z.3 r15))) ,_ ...)
                  (begin
                    (set! tmp-ra.1 r15)
                    (set! x.1 fv0)
                    (set! y.2 fv1)
